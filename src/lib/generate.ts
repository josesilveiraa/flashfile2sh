...
export default async function generate(input: string, output: string) {
  readFile(input, async (err, data) => {
    if (err?.code === 'ENOENT') throw new Error('Input file not found.');
    ...
    console.log(
      `Generating shellscript for ${flashing.header.software_version["@_version"]}`
    );
